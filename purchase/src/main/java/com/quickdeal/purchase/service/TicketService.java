...
public class TicketService {
  ...
  public void validateTicketAndPaymentPageAccessible(QueueMessage message)
    ...
    // 페이지 접속자 확인
    validatePaymentPageAccessWithRetryLimit(message.userId(), message.productId(), message.ticketNumber());
  }

  // :: 페이지 액세스 확인 with 재실행
  public void validatePaymentPageAccessWithRetryLimit(String userId, Long productId, Long ticketNumber)
      throws InterruptedException {
    for (int i = 0; i < retryLimit; i++) {
      try {
        ...
        if (inMemoryService.validatePaymentPageAccess(keys, args)) {
